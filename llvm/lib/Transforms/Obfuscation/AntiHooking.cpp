...
struct AntiHook : public ModulePass {
        ...
        if (AntiRebindSymbolTemp)
          for (Instruction &I : instructions(F))
            if (isa<CallInst>(&I) || isa<InvokeInst>(&I)) {
              ...
              IRBuilder<> Builder(&I); // Only declare once
              Type *TargetType = PointerType::get(IntegerType::get(Builder.getContext(), 8), 0);
              Value *Callee = CS.getCalledValue();
              Value *BitCasted = Builder.CreateBitCast(Callee, TargetType);
  ...
  void HandleInlineHookAArch64(Function *F) {
    ...
    Type *Int32Ty = Type::getInt32Ty(F->getContext());
      Type *Int32PtrTy = PointerType::get(Type::getInt32Ty(F->getContext()), 0);
